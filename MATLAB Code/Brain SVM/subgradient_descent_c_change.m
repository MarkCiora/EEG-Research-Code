function [w, b] = subgradient_descent_c_change(x, y, c, type, pow_gam)
% margin = 2/|w|
% offset = b/|w|
    ...
    while 1 == 1
        ...
        if transpose(prev - accuracy )*(prev - accuracy) < .005^2 / 2
            % if 2 < margin < 10
            % smaller c -> increase margin size
            % largeer c -> decrease margin size
            if (accuracy(2) - accuracy(1) + accuracy(3) - accuracy(4)) < 2
                c = c*.99;
            elseif (accuracy(2) - accuracy(1) + accuracy(3) - accuracy(4)) > 10
                c = c*1.01;
            else
                break
            end
